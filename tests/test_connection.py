...
class TestOperationQueue:
    @pytest.mark.asyncio
    async def test_queue_returns_future(self):
        """queue() should return an asyncio.Future."""
        q = OperationQueue()
        future = await q.queue(1, "Page.navigate", {"url": "https://x.com"})
        assert isinstance(future, asyncio.Future)
        assert not future.done()

    @pytest.mark.asyncio
    async def test_get_all_clears(self):
        """get_all() should clear the queue."""
        q = OperationQueue()
        await q.queue(1, "m1", None)
        await q.queue(2, "m2", None)
        ops = await q.get_all()
        assert len(ops) == 2
    ...
    @pytest.mark.asyncio
    async def test_get_all_preserves_order(self):
        """get_all() should preserve insertion order."""
        q = OperationQueue()
        await q.queue(1, "first", None)
        await q.queue(2, "second", None)
        await q.queue(3, "third", None)
        ops = await q.get_all()
